...
def main(args):
    ...
    fnr = fn / i1
    fpr = fp / i2
    # print("fnr: %f, fpr: %f, (fnr+fpr)/2: %f" % (fnr, fpr, (fnr+fpr)/2))
    
    # Write CSV to standard output
    sys.stdout.write("%s%d,%d,%d,%d,%d,%d,%d,%f,%f,%f\n" \
        % (prefix, n1, n2, nl, i1, i2, fn, fp, fnr, fpr, (fnr+fpr)/2))

    # print("treee 1 newick: %s\n" % tree1.newick())
    # print("treee 2 newick: %s\n" % tree2.newick())
